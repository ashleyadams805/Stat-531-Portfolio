...
```{r alpha}
#Code for question 5!
##alpha=0.1, or 0.5, alpha values from past stats classes LOL
ggplot(data = surveys, mapping = aes(x = weight, y = hindfoot_length)) + 
...
ggplot(data = surveys, 
   ...
   geom_boxplot() + 
    xlab("Weight (g)") + 
  ylab("Species")
## what happened with geom_jitter?
```

```{r Code for question 9}
ggplot(data = surveys, 
       mapping = aes(x = weight, y = species)) +
  geom_jitter(col = "red") +
   geom_boxplot() + 
